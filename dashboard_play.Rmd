...
dt3 <- group_by(le_anova_data, life_expect_class) %>%
  dplyr::summarize(mean_mortality_rate = mean(mortality_rate), standard_error = std.error(mortality_rate)) %>%
  dplyr::arrange(desc(mean_mortality_rate))
...
### Plots
```{r}
ggplotly(ggplot(region_mort_tb1, aes(x=life_expectancy_birth, y=mortality_rate)) + geom_point() + facet_wrap(. ~ region, nrow = 2) + labs(y = "COVID-19 Mortality Rate", x = "Life Expectancy at Birth (years)", title = "Relationship Between COVID-19 Mortality Rate and Life Expectancy"))
...
ggplotly(ggplot(dt3, aes(life_expect_class, mean_mortality_rate)) + 
      ...
      theme_minimal()) %>%
        layout(title = list(text = paste0("ANOVA Result Showing Multiple Comparisons of Relationship Between", "<br>", "Mortality Rate and Life Expectancy Class",
                                    "<br>",
                                    "<sup>",
                                     "Means not sharing any letter are significantly different at the 5% significance level (Tukey's test)","</sup>")), titlefont=list(size = 15),  margin = list(l=50, r=50, b=100, t=100, pad=4)) 
